...
def makePlural(word):
    ...
    """
    return map(lambda x: x[-1] + 's',word)
...
# TODO: Replace <FILL IN> with appropriate code
pluralLambdaRDD = wordsRDD.map(lambda x: ''.join((x,'s')))
...
#shakeWordsRDD = shakespeareWordsRDD.filter(None)
#shakeWordsRDD = shakespeareWordsRDD.filter(bool)
#shakeWordsRDD = shakespeareWordsRDD.filter(len)
